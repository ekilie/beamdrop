...
| **In-memory token revocation list is lost on restart** | Revoked JTIs (`pkg/auth/password.go`) exist only in memory. A restart also regenerates the JWT secret, which invalidates all tokens anyway, so this is safe. |
