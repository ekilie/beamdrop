...
| **API keys & secrets** | High | SQLite DB (SHA-256 hashed secrets in `APIKey` table) | Unauthorized API access, data exfiltration |
...
| 2 | **JWT token theft (XSS)** | External | Session token | CSP header, `httpOnly` cookies, `X-Content-Type-Options: nosniff` | Low — audit CSP for `unsafe-inline`/`unsafe-eval` allowances |
| 3 | **API key leakage** | Authenticated, Insider | API key secrets | Keys stored as SHA-256 hashes; secrets shown once at creation | Low — rotate keys periodically; monitor for leaked keys |
...
| 8 | **Cross-site request forgery (CSRF)** | External | Authenticated sessions | CORS disabled by default; `SameSite` cookie attributes | Low — validate `Origin` header on state-changing requests |
...
| 17 | **IP spoofing (rate limit bypass)** | External, Bot | Rate limiter effectiveness | `X-Forwarded-For` / `X-Real-IP` header inspection | Medium — trust proxy headers only from known reverse proxies |
...
| JWT tokens | HS256, 24-hour expiry, random 256-bit secret per process | `pkg/auth/password.go` |
| API key authentication | HMAC-SHA256 request signing, 15-min timestamp window | `pkg/crypto/signature.go` |
| API key storage | SHA-256 hashed secrets in database | `pkg/db/models.go` |
...
| CSP header | Restricts script, style, image, font, and connect sources | `pkg/middleware/security.go` |
...
| Control | Implementation | Reference |
|---------|---------------|-----------|
| Token-bucket rate limiter | Per-IP, 3 tiers (general, auth, upload) | `pkg/middleware/ratelimit.go` |
